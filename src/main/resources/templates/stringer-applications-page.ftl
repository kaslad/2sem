...
<h3 class="w3l_header w3_agileits_header"><span>Натяжки</span>
                ...
                <ul class="list-group w3-agile">
                    <#list model.applications as app >
                        <li class="list-group-item"><a href="#" data-toggle="modal"
                                                       data-target="#myModal"><#if app.name??>${app.name}</#if> ${app.horizontalWeight}
                            /${app.verticalWeight} <p>05 January 2017</p></a>
                            <a href="update?id=${app.id}&state=${model.state}"><span class="label label-success" style="float:right">
                                <#if model.state == 'NEW'>Взять</#if>
                                <#if model.state == 'PROGRESS'>Сделано</#if>
                                <#if model.state == 'REVIEW'>Связь</#if>
                                <#if model.state == 'ARCHIVE'>Х</#if>
                            </span></a></li>
